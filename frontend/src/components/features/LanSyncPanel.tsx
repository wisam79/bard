...
export const LanSyncPanel: React.FC<LanSyncPanelProps> = ({ notify }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className={`p-6 rounded-2xl border shadow-sm transition-all ${mode === 'server' ? 'bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20' : mode === 'client' ? 'bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20' : 'bg-brand-surface/40 dark:bg-white/[0.06] border-brand-border/30 dark:border-white/[0.08]'}`}>
        <div className="flex items-center gap-4">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${mode === 'server' ? 'bg-emerald-500 text-white shadow-emerald-500/30' : mode === 'client' ? 'bg-blue-500 text-white shadow-blue-500/30' : 'bg-brand-border/20 dark:bg-white/[0.06] text-brand-muted/50 dark:text-white/20'}`}>
          ...
          <div>
            ...
            <p className="text-sm font-medium text-brand-muted/60 dark:text-white/40">
      ...
      {mode !== 'client' && (
        <div className="bg-brand-surface/30 dark:bg-white/[0.06] backdrop-blur-xl border border-brand-border/30 dark:border-white/[0.08] rounded-2xl p-6 shadow-sm">
          ...
          {serverStatus?.running ? (
            <div className="space-y-6">
              <div className="p-5 bg-brand-surface/50 dark:bg-white/[0.05] border border-brand-border/30 dark:border-white/[0.08] rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-bold text-brand-muted/50 dark:text-white/20 mb-2">عنوان IP للاتصال</p>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-mono text-brand-muted/45 dark:text-white/30">Port: {serverStatus.port}</span>
                  </div>
                </div>
                <button onClick={copyAddress} className="p-3 bg-brand-border/15 dark:bg-white/[0.05] hover:bg-emerald-500 hover:text-white rounded-xl transition-all group border border-brand-border/35 dark:border-white/[0.09]" title="نسخ العنوان">
                  {copied ? <Check size={20} /> : <Copy size={20} />}
                </button>
              </div>

              <div className="bg-brand-surface/30 dark:bg-white/[0.06] rounded-2xl border border-brand-border/30 dark:border-white/[0.08] overflow-hidden">
                <div className="flex border-b border-brand-border/30 dark:border-white/[0.07]">
                  <button onClick={() => setActiveTab('clients')} className={`flex-1 py-3 text-sm font-bold transition-all ${activeTab === 'clients' ? 'bg-brand-surface/50 dark:bg-white/[0.06] text-primary-500 border-t-2 border-primary-500' : 'text-brand-muted/45 dark:text-white/30'}`}>
                    <Users size={14} className="inline ml-2" /> المتصلون ({connectedClients.length})
                  </button>
                  <button onClick={() => setActiveTab('blocked')} className={`flex-1 py-3 text-sm font-bold transition-all ${activeTab === 'blocked' ? 'bg-brand-surface/50 dark:bg-white/[0.06] text-red-500 border-t-2 border-red-500' : 'text-brand-muted/45 dark:text-white/30'}`}>
                ...
                <div className="p-4 min-h-[200px] max-h-[300px] overflow-y-auto">
                  {activeTab === 'clients' ? (
                    connectedClients.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-48 text-brand-muted/40 dark:text-white/25">
                      ...
                      <div className="space-y-2">
                        {connectedClients.map((client) => (
                          <div key={client.deviceId} className="flex items-center justify-between p-3 bg-brand-surface/30 dark:bg-white/[0.06] rounded-xl border border-brand-border/25 dark:border-white/[0.09]">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-brand-muted/40 dark:text-white/25">
                                  <span className="font-mono bg-brand-border/10 dark:bg-white/[0.06] px-1.5 rounded">{client.ipAddress}</span>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => handleBlockDevice(client.deviceId, client.deviceName)} className="p-2 bg-brand-border/10 dark:bg-white/[0.06] hover:bg-slate-800 text-brand-muted/45 dark:text-white/30 hover:text-white rounded-lg transition-colors" title="حظر"><Shield size={14} /></button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    blockedDevices.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-48 text-brand-muted/40 dark:text-white/25"><Shield size={32} className="mb-2" /><p>لا توجد أجهزة محظورة</p></div>
                    ) : (
                      <div className="space-y-2">
                        {blockedDevices.map((device) => (
                          <div key={device.id} className="flex items-center justify-between p-3 bg-red-500/5 border border-red-500/10 rounded-xl">
                            ...
                            <button onClick={() => handleUnblockDevice(device.id)} className="p-2 bg-brand-surface/30 hover:bg-emerald-500 hover:text-white text-brand-muted/45 dark:text-white/30 rounded-lg transition-colors" title="إلغاء الحظر"><Trash2 size={14} /></button>
                          </div>
                        ))}
                      </div>
                    )
            ...
            <div className="text-center py-8">
              <p className="text-brand-muted/50 dark:text-white/30 mb-6 max-w-md mx-auto">قم بتحويل هذا الجهاز إلى خادم رئيسي للسماح للأجهزة الأخرى بالاتصال ومزامنة البيانات.</p>
      ...
      {mode === 'standalone' && (
        <div className="flex items-center gap-4 px-8 opacity-50">
          ...
          <span className="text-xs font-bold text-brand-muted/40 dark:text-white/25 uppercase tracking-widest">خيارات الاتصال</span>
          <div className="flex-1 h-px bg-gradient-to-r from-transparent via-brand-border/20 dark:via-white/[0.06] to-transparent" />
        </div>
      )}
      ...
      {mode !== 'server' && (
        <div className="bg-brand-surface/30 dark:bg-white/[0.06] backdrop-blur-xl border border-brand-border/30 dark:border-white/[0.08] rounded-2xl p-6 shadow-sm">
          ...
          {mode === 'client' ? (
            <div className="space-y-6">
              <div className="p-5 bg-blue-500/5 border border-blue-500/15 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm text-brand-muted/50 dark:text-white/20 mb-1">متصل بالخادم:</p>
                    <code className="text-xl font-mono font-bold text-blue-500 tracking-wider" dir="ltr">{clientStatus?.serverAddress}</code>
                  </div>
                </div>
              </div>
              <button onClick={handleDisconnect} disabled={loading} className="w-full py-4 bg-brand-surface/30 hover:bg-red-500 hover:text-white text-brand-muted/50 dark:text-white/30 border border-brand-border/30 dark:border-white/[0.08] rounded-xl font-bold transition-all flex items-center justify-center gap-2">
                <WifiOff size={18} /> قطع الاتصال بالخادم
              </button>
            </div>
          ) : showConnectForm ? (
            <div className="bg-brand-surface/30 dark:bg-white/[0.06] rounded-2xl border border-brand-border/30 dark:border-white/[0.08] p-6 space-y-4">
              ...
              <div className="min-h-[100px] max-h-[200px] overflow-y-auto space-y-2">
                {isScanning ? (
                  <div className="flex flex-col items-center justify-center py-8 text-brand-muted/40 dark:text-white/25"><RefreshCw size={24} className="animate-spin mb-2 text-primary-500" /><p className="text-xs">جاري البحث عن الخوادم...</p></div>
                ) : discoveredServers.length > 0 ? (
                  discoveredServers.map((server, idx) => (
                    <button key={idx} onClick={() => { setSelectedServer(server); setManualIp(''); }} className={`w-full p-4 rounded-xl border text-right transition-all ${selectedServer?.serverIP === server.serverIP ? 'bg-blue-500/10 border-blue-500 shadow-md shadow-blue-500/10' : 'bg-brand-dark/35 dark:bg-white/[0.06] border-brand-border/30 dark:border-white/[0.07] hover:border-blue-500/50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-brand-muted/40 dark:text-white/25">تم اكتشافه تلقائياً</span>
                          </div>
                        </div>
                        <code className="text-xs font-mono bg-brand-border/10 dark:bg-white/[0.06] px-2 py-1 rounded border border-brand-border/30 dark:border-white/[0.09]">{server.serverIP}</code>
                      </div>
                    </button>
                  ))
                ) : (
                  <div className="py-8 text-center bg-brand-dark/35 dark:bg-white/[0.06] rounded-xl border border-dashed border-brand-border/35 dark:border-white/[0.09] text-brand-muted/40 dark:text-white/25">
                    <Ban size={24} className="mx-auto mb-2 opacity-50" />
                    <p className="text-xs">لم يتم العثور على خوادم. تأكد أن الخادم يعمل.</p>
                  </div>
                )}
              </div>
              <div className="h-px bg-brand-border/15 dark:bg-white/[0.06] my-2" />
              <div className="flex items-center gap-2 mb-2"><Terminal size={14} className="text-brand-muted/40 dark:text-white/25" /><span className="text-xs font-bold text-brand-muted/40 dark:text-white/25">أو أدخل العنوان يدوياً:</span></div>
              <input type="text" value={manualIp} onChange={(e) => { setManualIp(e.target.value); setSelectedServer(null); }} placeholder="مثال: 192.168.1.100" dir="ltr" className="w-full p-3 bg-brand-dark/35 dark:bg-white/[0.06] border border-brand-border/30 dark:border-white/[0.09] rounded-xl text-center font-mono text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all" />
              <div className="flex gap-3 mt-4">
                <button onClick={() => setShowConnectForm(false)} className="flex-1 py-3 bg-brand-surface/30 hover:bg-brand-border/20 text-brand-accent dark:text-white/60 border border-brand-border/30 dark:border-white/[0.09] rounded-xl font-bold transition-colors">إلغاء</button>
            ...
            <div className="text-center py-8">
              <p className="text-brand-muted/50 dark:text-white/30 mb-6 max-w-md mx-auto">اربط هذا الجهاز بشبكة المحل للعمل كنقطة بيع متصلة بالخادم الرئيسي.</p>
      ...
      <div className="p-4 bg-blue-500/5 rounded-2xl border border-blue-500/10 flex items-start gap-3">
        ...
        <div className="text-xs text-brand-muted/50 dark:text-white/30 leading-relaxed">
