...
const CustomersSubTab: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (customer: Partial<Customer>) => wailsApp.CreateCustomer(useAuthStore.getState().token || '', customer as Customer),
  ...
  const updateMutation = useMutation({
    mutationFn: (customer: Partial<Customer>) => wailsApp.UpdateCustomer(useAuthStore.getState().token || '', customer as Customer),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => wailsApp.DeleteCustomer(useAuthStore.getState().token || '', id),
      ...
      <div className="bg-brand-surface border border-brand-border/30 rounded-3xl flex-1 flex flex-col overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-brand-border/30">
          <div className="relative w-full max-w-md group">
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-muted/50 group-focus-within:text-primary-400 transition-colors" />
            <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setPage(1); }} className="input pr-12 py-3 bg-brand-dark/35 border-brand-border/35 focus:bg-brand-dark/40" placeholder="بحث باسم العميل، رقم الهاتف..." />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-brand-muted/50 space-y-4"><Users className="w-10 h-10 opacity-50" /><p className="font-bold">لم يتم العثور على عملاء</p></div>
          ) : (
            <table className="w-full text-right">
              <thead><tr className="bg-brand-dark/45 border-b border-brand-border/30">
                <th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest">معلومات العميل</th>
                <th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest">رقم الهاتف</th>
                <th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest">الدين المستحق</th>
                <th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest text-center">النقاط</th>
              ...
              <tbody className="divide-y divide-brand-border/10">
                {customers.map((customer: Customer) => (
                  <tr key={customer.id} className="hover:bg-brand-dark/35 transition-colors group">
                    <td className="py-4 px-6"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary-500/10 flex items-center justify-center text-primary-400 font-black text-xs">{customer.name.charAt(0)}</div><div><p className="text-sm font-bold dark:text-white text-gray-900 leading-none mb-1 group-hover:text-primary-400 transition-colors">{customer.name}</p><p className="text-[10px] text-brand-muted/60 font-medium truncate max-w-[150px]">{customer.notes || 'لا توجد ملاحظات'}</p></div></div></td>
                    <td className="py-4 px-6"><span className="text-xs font-mono font-bold text-brand-accent/70 bg-brand-dark/40 px-3 py-1 rounded-lg border border-brand-border/35">{customer.phone}</span></td>
                    ...
                    <td className="py-4 px-6"><div className="flex items-center justify-center gap-2">
                      <button onClick={() => setViewingCustomer(customer)} className="p-2 rounded-xl bg-brand-dark/40 border border-brand-border/30 text-brand-muted/60 hover:text-white hover:border-primary-500/50 hover:bg-primary-500/10 transition-all"><Eye size={16} /></button>
                      <button onClick={() => handleOpenModal(customer)} className="p-2 rounded-xl bg-brand-dark/40 border border-brand-border/30 text-brand-muted/60 hover:text-white hover:border-primary-500/50 hover:bg-primary-500/10 transition-all"><Pencil size={16} /></button>
            ...
            {viewingCustomer ? (
              <>
                ...
                <div className="space-y-4">
                  <div className="bg-brand-dark/45 rounded-2xl p-4 border border-brand-border/30">
                    <p className="text-[10px] font-black text-brand-muted/50 uppercase tracking-widest mb-3">الوضع المالي</p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-brand-dark/45 rounded-2xl p-4 border border-brand-border/30"><p className="text-[10px] font-black text-brand-muted/50 uppercase tracking-widest mb-1">المشتريات</p><p className="text-base font-black dark:text-white text-gray-900 truncate">{viewingCustomer.totalPurchases.toLocaleString('ar-IQ')}</p></div>
                    <div className="bg-brand-dark/45 rounded-2xl p-4 border border-brand-border/30 text-center"><p className="text-[10px] font-black text-brand-muted/50 uppercase tracking-widest mb-1">نقاط الولاء</p><p className="text-base font-black text-primary-400">{viewingCustomer.points}</p></div>
                  </div>
                  {viewingCustomer.lastVisit && <div className="flex items-center gap-2 text-[10px] font-bold text-brand-muted/60 justify-center"><Clock size={12} />آخر زيارة: {viewingCustomer.lastVisit}</div>}
                </div>
              ...
              <>
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button onClick={handleCloseModal} className="text-brand-muted/50 hover:dark:text-white text-gray-900 transition-colors"><Plus className="rotate-45" size={32} /></button>
                </div>
...
export default CustomersSubTab;
