      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-brand-muted/40 dark:text-white/25 font-medium">تنظيم طاولات المطعم وإدارة الحجوزات</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex bg-brand-surface/30 dark:bg-white/[0.06] border border-brand-border/30 dark:border-white/[0.08] rounded-xl p-0.5">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-primary-500 text-white' : 'text-brand-muted/45 dark:text-white/30'}`}><LayoutGrid size={16} /></button>
            <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-primary-500 text-white' : 'text-brand-muted/45 dark:text-white/30'}`}><List size={16} /></button>
          </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="stat-card flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-brand-muted/45 dark:text-white/30">{s.label}</span>
          </div>
        ))}
      </div>

      <div className="flex items-center gap-2 flex-wrap">
        <select value={filterFloor} onChange={e => setFilterFloor(e.target.value)} className="px-3 py-2 bg-brand-surface/40 dark:bg-white/[0.05] border border-brand-border/35 dark:border-white/[0.08] rounded-xl text-xs font-bold text-brand-muted/60 dark:text-white/20 outline-none focus:border-primary-500/40">
          {floors.map(f => <option key={f} value={f === 'الكل' ? 'all' : f}>{f}</option>)}
        </select>
        {(['all', 'available', 'occupied', 'reserved', 'cleaning'] as const).map(s => (
          <button key={s} onClick={() => setFilterStatus(s)} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${filterStatus === s ? 'bg-primary-500 border-primary-500 text-white' : 'bg-brand-surface/35 dark:bg-white/[0.06] border-brand-border/25 dark:border-white/[0.07] text-brand-muted/45 dark:text-white/30 hover:border-brand-border/30'}`}>
        ...
        {filtered.map(table => {
              ...
              <div key={table.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all hover:shadow-md ${cfg.color}`}>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    {table.name && <span className="text-xs text-brand-muted/50 dark:text-white/30">({table.name})</span>}
                  </div>
                  <span className="text-xs text-brand-muted/45 dark:text-white/25 flex items-center gap-1"><Users size={12} /> {table.seats}</span>
                  <span className="text-xs text-brand-muted/40 dark:text-white/25">{table.floor}</span>
                  {table.assignedWaiter && <span className="text-xs text-brand-muted/40 dark:text-white/25">الويتر: {table.assignedWaiter}</span>}
                </div>
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-lg text-[9px] font-bold border ${cfg.color}`}>{cfg.label}</span>
                  <button onClick={() => openEdit(table)} className="p-1.5 rounded-lg hover:bg-brand-border/20 dark:hover:bg-white/[0.06] text-brand-muted/40 dark:text-white/25 hover:text-primary-500 transition-colors"><Edit2 size={14} /></button>
                </div>
              </div>
            );
          }
          return (
            <div key={table.id} className={`relative p-4 rounded-2xl border-2 transition-all hover:shadow-lg hover:-translate-y-0.5 cursor-pointer group ${cfg.color}`} onClick={() => {
              ...
              {table.name && <p className="text-xs font-bold text-brand-muted/60 dark:text-white/20 mb-2">{table.name}</p>}
              <div className="flex items-center gap-2 text-[10px] text-brand-muted/40 dark:text-white/25 mb-2">
                <Users size={10} /> {table.seats} مقاعد
              </div>
              {table.assignedWaiter && <p className="text-[9px] text-brand-muted/35 dark:text-white/8">الويتر: {table.assignedWaiter}</p>}
              {table.notes && <p className="text-[9px] text-brand-muted/35 dark:text-white/8 truncate mt-1">{table.notes}</p>}
              <div className="absolute bottom-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={e => { e.stopPropagation(); openEdit(table); }} className="p-1 rounded bg-brand-surface/40 dark:bg-white/[0.06] text-brand-muted/50 dark:text-white/30 hover:text-primary-500"><Edit2 size={10} /></button>
                <button onClick={e => { e.stopPropagation(); handleDelete(table.id); }} className="p-1 rounded bg-brand-surface/40 dark:bg-white/[0.06] text-brand-muted/50 dark:text-white/30 hover:text-red-500"><Trash2 size={10} /></button>
              </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-brand-muted/50 dark:text-white/30 uppercase tracking-[0.1em]">رقم الطاولة *</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-brand-muted/50 dark:text-white/30 uppercase tracking-[0.1em]">عدد المقاعد</label>
              <input type="number" value={form.seats} onChange={e => setForm({ ...form, seats: e.target.value })} className="input py-2.5 text-center" min="1" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-brand-muted/50 dark:text-white/30 uppercase tracking-[0.1em]">اسم الطاولة (اختياري)</label>
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-brand-muted/50 dark:text-white/30 uppercase tracking-[0.1em]">الطابق / المنطقة</label>
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-brand-muted/50 dark:text-white/30 uppercase tracking-[0.1em]">ملاحظات</label>
...
const KitchenTab: React.FC = () => {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      default: return 'border-brand-border/35 bg-brand-surface';
    }
  };
  ...
  const getPriorityIcon = (priority: string) => {
    switch (priority) {
      ...
      default: return <Clock className="w-4 h-4 text-brand-muted/60" />;
    }
  };
    ...
    <div className="h-full flex flex-col p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 flex-1 overflow-auto">
        ...
        {orders.filter(o => o.status !== 'completed' && o.status !== 'cancelled').length === 0 && (
          <div className="col-span-full text-center py-20 text-brand-muted/50">
    ...
    <div className="flex-1 overflow-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold">كيوسك الخدمة الذاتية</h1><p className="text-xs text-brand-muted/60 dark:text-white/30">إعداد شاشات الخدمة الذاتية للعملاء</p></div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-3">
          ...
          {(layouts || []).map((l: KioskLayout) => (
            <div key={l.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 dark:border-white/[0.07] p-4 flex items-center justify-between">
              <div>
                <p className="font-semibold text-sm">{l.name}</p>
                <p className="text-xs text-brand-muted/60 dark:text-white/30">{l.theme} • {l.fontSize} • {l.welcomeMsg}</p>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="space-y-3">
          ...
          {(sessions || []).map((s: KioskSession) => (
            <div key={s.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-emerald-500/20 p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-brand-muted/60 dark:text-white/30">بدأت: {s.startedAt} • {s.status}</p>
              </div>
          ...
          {(!sessions || sessions.length === 0) && <p className="text-center text-brand-muted/50 dark:text-white/20 py-4 text-sm">لا توجد جلسات نشطة</p>}
        </div>
      </div>

      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="تصميم كيوسك جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">السمة</label><select className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={form.theme} onChange={e => setForm({ ...form, theme: e.target.value as 'light' | 'dark' | 'colorful' })}><option value="light">فاتح</option><option value="dark">داكن</option><option value="colorful">ملون</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">حجم الخط</label><select className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={form.fontSize} onChange={e => setForm({ ...form, fontSize: e.target.value as 'small' | 'medium' | 'large' | 'xlarge' })}><option value="small">صغير</option><option value="medium">متوسط</option><option value="large">كبير</option><option value="xlarge">كبير جداً</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">رسالة الترحيب</label><input className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={form.welcomeMsg} onChange={e => setForm({ ...form, welcomeMsg: e.target.value })} /></div>
          <div className="flex items-center gap-4">
  ...
  return (
    <div className="flex-1 overflow-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-brand-muted/60 dark:text-white/30">تتبع أداء الموظفين وحساب العمولات</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 dark:border-white/[0.07] p-4">
          <div className="flex items-center gap-2 mb-1"><TrendingUp size={14} className="text-emerald-500" /><span className="text-xs text-brand-muted/60 dark:text-white/30">إجمالي المبيعات</span></div>
          <p className="text-lg font-bold">{stats.totalSales.toLocaleString()}</p>
        </div>
        <div className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 dark:border-white/[0.07] p-4">
          <div className="flex items-center gap-2 mb-1"><Users size={14} className="text-blue-500" /><span className="text-xs text-brand-muted/60 dark:text-white/30">عدد الموظفين</span></div>
          <p className="text-lg font-bold">{stats.totalStaff}</p>
        </div>
        <div className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 dark:border-white/[0.07] p-4">
          <div className="flex items-center gap-2 mb-1"><DollarSign size={14} className="text-purple-500" /><span className="text-xs text-brand-muted/60 dark:text-white/30">إجمالي العمولات</span></div>
      ...
      <div className="flex gap-2">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === tab.id ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30 hover:bg-brand-surface/30'}`}>{tab.label}</button>
        ))}
      </div>
          ...
          {(rules || []).map((r: CommissionRule) => (
            <div key={r.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 dark:border-white/[0.07] p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-brand-muted/60 dark:text-white/30">{r.type === 'percentage' ? `${r.value}%` : r.value.toLocaleString()} • الحد الأدنى: {r.minAmount.toLocaleString()}</p>
              </div>
              <div className="flex items-center gap-2">
      ...
      {activeTab === 'payments' && (
        <div className="text-center text-brand-muted/50 dark:text-white/20 py-8">
      ...
      <Modal isOpen={showRuleModal} onClose={() => setShowRuleModal(false)} title="قاعدة عمولة جديدة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={ruleForm.name} onChange={e => setRuleForm({ ...ruleForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">النوع</label><select className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={ruleForm.type} onChange={e => setRuleForm({ ...ruleForm, type: e.target.value as 'percentage' | 'fixed' })}><option value="percentage">نسبة مئوية</option><option value="fixed">مبلغ ثابت</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">القيمة</label><input type="number" className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={ruleForm.value} onChange={e => setRuleForm({ ...ruleForm, value: parseFloat(e.target.value) || 0 })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الحد الأدنى للمبيعات</label><input type="number" className="w-full bg-brand-surface/50 dark:bg-[#1e1e1e] border border-brand-border/35 dark:border-white/10 rounded-lg px-3 py-2 text-sm" value={ruleForm.minAmount} onChange={e => setRuleForm({ ...ruleForm, minAmount: parseFloat(e.target.value) || 0 })} /></div>
