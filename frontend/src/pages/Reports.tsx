      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          <div key={i} className="bg-brand-surface border border-brand-border/30 rounded-3xl p-6 flex items-center gap-5">
            ...
            <div>
              <p className="text-xs font-bold text-brand-muted/60 uppercase tracking-widest mb-1">{stat.label}</p>
...
const AnalyticsSubTab: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col p-6 space-y-6 overflow-auto bg-brand-dark/35">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-brand-surface rounded-xl p-5 border border-brand-border/35">
      ...
      <div className="bg-brand-surface rounded-xl p-5 border border-brand-border/35">
        ...
        <div className="overflow-x-auto">
          ...
          {demands.length === 0 && <p className="text-center text-brand-muted/60 py-8">لا توجد منتجات تحتاج إعادة طلب</p>}
        </div>
      </div>
    ...
    <div className="p-6 h-full flex flex-col gap-6 relative overflow-hidden bg-brand-dark/35">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button onClick={() => setActiveSubTab('templates')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'templates' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30'}`}>قوالب التقارير</button>
          <button onClick={() => setActiveSubTab('scheduled')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'scheduled' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30'}`}>التصدير المجدول</button>
        </div>
        <div className="flex gap-2">
      ...
      {activeSubTab === 'templates' && (
        <div className="grid grid-cols-2 gap-4">
          {(templates || []).map((t: ReportTemplate) => (
            <div key={t.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 p-4">
              ...
              <p className="text-xs text-brand-muted/60 dark:text-white/30 mb-3">{t.description || 'بدون وصف'}</p>
          ...
          {(!templates || templates.length === 0) && <p className="text-center text-brand-muted/50 dark:text-white/20 py-8 col-span-2">لا توجد قوالب تقارير</p>}
        </div>
      )}
        ...
        <div className="grid gap-3">
          {(scheduled || []).map((s: ScheduledExport) => (
            <div key={s.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 p-4 flex items-center justify-between">
              <div className="flex items-center gap-3"><Clock size={16} className="text-indigo-500" /><div><p className="font-semibold text-sm">{s.name}</p><p className="text-xs text-brand-muted/60">{s.format} • {s.frequency} • {s.recipients}</p></div></div>
          ...
          {(!scheduled || scheduled.length === 0) && <p className="text-center text-brand-muted/50 dark:text-white/20 py-8">لا يوجد تصدير مجدول</p>}
        </div>
      )}

      <Modal isOpen={showTemplateModal} onClose={() => setShowTemplateModal(false)} title="تقرير جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.name} onChange={e => setTemplateForm({ ...templateForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الوصف</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.description} onChange={e => setTemplateForm({ ...templateForm, description: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">النوع</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.type} onChange={e => setTemplateForm({ ...templateForm, type: e.target.value as 'sales' | 'inventory' | 'financial' | 'custom' })}><option value="sales">مبيعات</option><option value="inventory">مخزون</option><option value="financial">مالي</option><option value="custom">مخصص</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">نوع الرسم البياني</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.chartType} onChange={e => setTemplateForm({ ...templateForm, chartType: e.target.value as 'bar' | 'line' | 'pie' | 'table' })}><option value="table">جدول</option><option value="bar">أعمدة</option><option value="line">خطي</option><option value="pie">دائري</option></select></div>
      ...
      <Modal isOpen={showScheduleModal} onClose={() => setShowScheduleModal(false)} title="تصدير مجدول جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={scheduleForm.name} onChange={e => setScheduleForm({ ...scheduleForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">التقرير</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={scheduleForm.reportId} onChange={e => setScheduleForm({ ...scheduleForm, reportId: e.target.value })}><option value="">اختر التقرير</option>{(templates || []).map((t: ReportTemplate) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
          <div><label className="text-xs font-semibold mb-1 block">الصيغة</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={scheduleForm.format} onChange={e => setScheduleForm({ ...scheduleForm, format: e.target.value as 'pdf' | 'csv' | 'excel' })}><option value="pdf">PDF</option><option value="csv">CSV</option><option value="excel">Excel</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">التكرار</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={scheduleForm.frequency} onChange={e => setScheduleForm({ ...scheduleForm, frequency: e.target.value as 'daily' | 'weekly' | 'monthly' })}><option value="daily">يومي</option><option value="weekly">أسبوعي</option><option value="monthly">شهري</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">المستلمون</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="email1@example.com, email2@example.com" value={scheduleForm.recipients} onChange={e => setScheduleForm({ ...scheduleForm, recipients: e.target.value })} /></div>
...
const MessagingSubTab: React.FC = () => {
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 relative overflow-hidden bg-brand-dark/35">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button onClick={() => setActiveSubTab('providers')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'providers' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30 hover:bg-brand-surface/50'}`}><Phone size={14} /> مزودو الخدمة</button>
          <button onClick={() => setActiveSubTab('templates')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'templates' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30 hover:bg-brand-surface/50'}`}><MessageSquare size={14} /> القوالب</button>
          <button onClick={() => setActiveSubTab('logs')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'logs' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30 hover:bg-brand-surface/50'}`}><Send size={14} /> سجل الرسائل</button>
        </div>
        <div className="flex gap-2">
      ...
      {activeSubTab === 'providers' && (
        <div className="grid gap-3">
          ...
          {(!providers || providers.length === 0) && <p className="text-center text-brand-muted/50 dark:text-white/20 py-8">لا يوجد مزودو خدمة</p>}
        </div>
      )}
        ...
        <div className="grid gap-3">
          ...
          {(templates || []).map((t: MessageTemplate) => (
            <div key={t.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 p-4">
              ...
              <p className="text-xs text-brand-muted/60 line-clamp-2">{t.content}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {activeSubTab === 'logs' && (
        <div className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-brand-border/25"><th className="text-right px-4 py-3 font-semibold text-brand-accent/50">المستلم</th><th className="text-right px-4 py-3 font-semibold text-brand-accent/50">المحتوى</th><th className="text-right px-4 py-3 font-semibold text-brand-accent/50">الحالة</th></tr></thead>
            <tbody>
      ...
      <Modal isOpen={showProviderModal} onClose={() => setShowProviderModal(false)} title="مزود خدمة جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={providerForm.name} onChange={e => setProviderForm({ ...providerForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">النوع</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={providerForm.type} onChange={e => setProviderForm({ ...providerForm, type: e.target.value as 'whatsapp' | 'sms' | 'telegram' })}><option value="whatsapp">واتساب</option><option value="sms">رسائل SMS</option><option value="telegram">تلغرام</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">مفتاح API</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={providerForm.apiKey} onChange={e => setProviderForm({ ...providerForm, apiKey: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">رقم الهاتف</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={providerForm.phone} onChange={e => setProviderForm({ ...providerForm, phone: e.target.value })} /></div>
      ...
      <Modal isOpen={showTemplateModal} onClose={() => setShowTemplateModal(false)} title="قالب رسالة جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.name} onChange={e => setTemplateForm({ ...templateForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">النوع</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={templateForm.type} onChange={e => setTemplateForm({ ...templateForm, type: e.target.value as 'receipt' | 'promotion' | 'reminder' | 'custom' })}><option value="receipt">إيصال</option><option value="promotion">ترويجي</option><option value="reminder">تذكير</option><option value="custom">مخصص</option></select></div>
          <div><label className="text-xs font-semibold mb-1 block">المحتوى</label><textarea className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm h-24 resize-none" value={templateForm.content} onChange={e => setTemplateForm({ ...templateForm, content: e.target.value })} placeholder="استخدم {{name}} للمتغيرات" /></div>
      ...
      <Modal isOpen={showSendModal} onClose={() => setShowSendModal(false)} title="إرسال رسالة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">مزود الخدمة</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={sendForm.providerId} onChange={e => setSendForm({ ...sendForm, providerId: e.target.value })}><option value="">اختر المزود</option>{(providers || []).map((p: MessagingProvider) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
          <div><label className="text-xs font-semibold mb-1 block">المستلم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="+9647700000000" value={sendForm.recipient} onChange={e => setSendForm({ ...sendForm, recipient: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">المحتوى</label><textarea className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm h-24 resize-none" value={sendForm.content} onChange={e => setSendForm({ ...sendForm, content: e.target.value })} /></div>
