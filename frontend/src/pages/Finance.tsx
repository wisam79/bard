...
const ExpensesSubTab: React.FC = () => {
  ...
  return (
    <div className="p-8 h-full flex flex-col bg-brand-dark/35">
      ...
      {activeSubTab === 'expenses' && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-brand-surface border border-brand-border/30 rounded-3xl p-6 flex items-center gap-5 shadow-xl"><div className="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-400 border border-red-500/20"><TrendingDown size={24} /></div><div><p className="text-xs font-bold text-brand-muted/60 uppercase tracking-widest mb-1">إجمالي المصروفات</p><p className="text-2xl font-black text-red-400">{totalAmount.toLocaleString('ar-IQ')} <span className="text-sm opacity-50">د.ع</span></p></div></div>
            <div className="bg-brand-surface border border-brand-border/30 rounded-3xl p-6 flex items-center gap-5 shadow-xl"><div className="w-14 h-14 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400 border border-blue-500/20"><Wallet size={24} /></div><div><p className="text-xs font-bold text-brand-muted/60 uppercase tracking-widest mb-1">عدد العمليات</p><p className="text-2xl font-black dark:text-white text-gray-900">{totalExpenses}</p></div></div>
          </div>
          ...
          <div className="bg-brand-surface border border-brand-border/30 rounded-3xl flex-1 flex flex-col overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-brand-border/30">
              <div className="relative w-full max-w-xs group"><Tag className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-muted/50 group-focus-within:text-primary-400" /><select value={selectedCategory} onChange={(e) => { setSelectedCategory(e.target.value); setPage(1); }} className="input pr-10 bg-brand-dark/35 border-brand-border/35 appearance-none"><option value="">كل فئات المصاريف</option>{expenseCategories.map((cat) => (<option key={cat} value={cat}>{cat}</option>))}</select></div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
              ...
              ) : expenses.length === 0 ? (<div className="flex flex-col items-center justify-center h-full text-brand-muted/50 space-y-4"><Wallet size={48} className="opacity-20" /><p className="font-bold">لا توجد سجلات مصروفات</p></div>
              ) : (
                <table className="w-full text-right border-collapse">
                  <thead><tr className="bg-brand-dark/45 border-b border-brand-border/30"><th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest">المصروف / التفاصيل</th><th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest text-center">الفئة</th><th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest text-center">المبلغ</th><th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest text-center">التاريخ</th><th className="py-4 px-6 text-[10px] font-black text-brand-muted/60 uppercase tracking-widest text-center">إجراءات</th></tr></thead>
                  <tbody className="divide-y divide-brand-border/10">
                    {expenses.map((expense: Expense) => (
                      <tr key={expense.id} className="hover:bg-brand-dark/35 transition-colors group">
                        <td className="py-4 px-6"><div><p className="text-sm font-bold dark:text-white text-gray-900 leading-none mb-1 group-hover:text-primary-400 transition-colors">{expense.title}</p><p className="text-[10px] text-brand-muted/60 font-medium truncate max-w-[200px]">{expense.notes || 'لا توجد ملاحظات'}</p></div></td>
                        <td className="py-4 px-6 text-center"><span className="text-[10px] font-black text-brand-accent/60 bg-brand-dark/40 px-3 py-1 rounded-full border border-brand-border/35">{expense.category || 'عام'}</span></td>
            ...
            {totalPages > 1 && (<div className="p-4 bg-brand-dark/35 border-t border-brand-border/30 flex items-center justify-center gap-4"><button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="btn-secondary px-4 py-2 text-xs">السابق</button><span className="text-xs font-bold text-brand-accent/50">صفحة {page} من {totalPages}</span><button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="btn-secondary px-4 py-2 text-xs">التالي</button></div>)}
          </div>
        </>
      )}

      {activeSubTab === 'payments' && (
        <div className="bg-brand-surface border border-brand-border/30 rounded-3xl flex-1 flex flex-col items-center justify-center text-brand-muted/50 space-y-4 shadow-xl"><TrendingUp size={64} className="opacity-10" /><div className="text-center"><h3 className="text-xl font-black dark:text-white text-gray-900 mb-1">وحدة المدفوعات</h3><p className="text-sm font-medium">هذه الوحدة قيد التطوير وستكون متاحة في التحديث القادم</p></div></div>
      )}
    ...
    <div className="p-6 h-full flex flex-col gap-6 relative overflow-hidden bg-brand-dark/35">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button onClick={() => setActiveSubTab('budgets')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'budgets' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30'}`}>الميزانيات</button>
          <button onClick={() => setActiveSubTab('workflows')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeSubTab === 'workflows' ? 'bg-primary-500/10 text-primary-500 border border-primary-500/20' : 'text-brand-muted/60 dark:text-white/30'}`}>سير الموافقات</button>
        </div>
        <div className="flex gap-2">
      ...
      {activeSubTab === 'budgets' && (
        <div className="grid gap-3">
          {(budgets || []).map((b: Budget) => {
            ...
            return (
              <div key={b.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 p-4">
                <div className="flex items-center justify-between mb-2"><p className="font-semibold text-sm">{b.name}</p><div className="flex items-center gap-2"><span className="text-xs text-brand-muted/60">{b.category} • {b.period}</span>{isOver && <AlertTriangle size={14} className="text-red-500" />}</div></div>
                <div className="flex items-center justify-between text-xs mb-1"><span className={isOver ? 'text-red-500 font-bold' : ''}>{b.spentAmount.toLocaleString()} / {b.amount.toLocaleString()}</span><span className="text-brand-muted/60">{pct.toFixed(0)}%</span></div>
      ...
      {activeSubTab === 'workflows' && (
        <div className="grid gap-3">
          {(workflows || []).map((w: ApprovalWorkflow) => (
            <div key={w.id} className="bg-brand-surface/50 dark:bg-[#1e1e1e]/50 rounded-xl border border-brand-border/30 p-4 flex items-center justify-between">
              <div><p className="font-semibold text-sm">{w.name}</p><p className="text-xs text-brand-muted/60">{w.minAmount.toLocaleString()} - {w.maxAmount > 0 ? w.maxAmount.toLocaleString() : '∞'} • مستوى: {w.requiredLevel}</p></div>
      ...
      <Modal isOpen={showBudgetModal} onClose={() => setShowBudgetModal(false)} title="ميزانية جديدة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={budgetForm.name} onChange={e => setBudgetForm({ ...budgetForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الفئة</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={budgetForm.category} onChange={e => setBudgetForm({ ...budgetForm, category: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">المبلغ</label><input type="number" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={budgetForm.amount} onChange={e => setBudgetForm({ ...budgetForm, amount: parseFloat(e.target.value) || 0 })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الفترة</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={budgetForm.period} onChange={e => setBudgetForm({ ...budgetForm, period: e.target.value as 'monthly' | 'quarterly' | 'yearly' })}><option value="monthly">شهري</option><option value="quarterly">ربع سنوي</option><option value="yearly">سنوي</option></select></div>
      ...
      <Modal isOpen={showWorkflowModal} onClose={() => setShowWorkflowModal(false)} title="سير موافقة جديد">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={workflowForm.name} onChange={e => setWorkflowForm({ ...workflowForm, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الحد الأدنى للمبلغ</label><input type="number" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={workflowForm.minAmount} onChange={e => setWorkflowForm({ ...workflowForm, minAmount: parseFloat(e.target.value) || 0 })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الحد الأقصى (0 = بلا حدود)</label><input type="number" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={workflowForm.maxAmount} onChange={e => setWorkflowForm({ ...workflowForm, maxAmount: parseFloat(e.target.value) || 0 })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">مستوى الموافقة</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={workflowForm.requiredLevel} onChange={e => setWorkflowForm({ ...workflowForm, requiredLevel: e.target.value as 'manager' | 'admin' })}><option value="manager">مدير</option><option value="admin">مدير النظام</option></select></div>
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 relative overflow-hidden bg-brand-dark/35">
      ...
      <div className="bg-brand-surface border border-brand-border/30 rounded-3xl overflow-hidden shadow-2xl">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {(!taxRates || taxRates.length === 0) && <tr><td colSpan={6} className="px-4 py-8 text-center text-brand-muted/50">لا توجد ضرائب مسجلة</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="ضريبة جديدة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="ضريبة القيمة المضافة" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الرمز</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="VAT" value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">المعدل %</label><input type="number" step="0.1" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={form.rate} onChange={e => setForm({ ...form, rate: parseFloat(e.target.value) || 0 })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">النوع</label><select className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}><option value="sales">مبيعات</option><option value="purchase">مشتريات</option><option value="withholding">خصم</option></select></div>
...
const CurrenciesSubTab: React.FC = () => {
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 relative overflow-hidden bg-brand-dark/35">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center"><Coins size={20} className="text-amber-500" /></div><div><h1 className="text-xl font-bold">العملات والصرف</h1><p className="text-xs text-brand-muted/60">إدارة العملات وأسعار الصرف</p></div></div>
        <div className="flex gap-2">
      ...
      <div className="bg-brand-surface border border-brand-border/30 rounded-3xl overflow-hidden shadow-2xl">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {(!currencies || currencies.length === 0) && <tr><td colSpan={6} className="px-4 py-8 text-center text-brand-muted/50">لا توجد عملات مسجلة</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="عملة جديدة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">الرمز</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="USD" value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الاسم</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="دولار أمريكي" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">الرمز</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="$" value={form.symbol} onChange={e => setForm({ ...form, symbol: e.target.value })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">سعر الصرف</label><input type="number" step="0.01" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={form.exchangeRate} onChange={e => setForm({ ...form, exchangeRate: parseFloat(e.target.value) || 0 })} /></div>
      ...
      <Modal isOpen={showConvertModal} onClose={() => { setShowConvertModal(false); setConvertResult(null); }} title="تحويل عملة">
        <div className="space-y-4 p-4">
          <div><label className="text-xs font-semibold mb-1 block">من عملة</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="USD" value={convertForm.fromCode} onChange={e => setConvertForm({ ...convertForm, fromCode: e.target.value.toUpperCase() })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">إلى عملة</label><input className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" placeholder="IQD" value={convertForm.toCode} onChange={e => setConvertForm({ ...convertForm, toCode: e.target.value.toUpperCase() })} /></div>
          <div><label className="text-xs font-semibold mb-1 block">المبلغ</label><input type="number" className="w-full bg-brand-surface/50 border border-brand-border/35 rounded-lg px-3 py-2 text-sm" value={convertForm.amount} onChange={e => setConvertForm({ ...convertForm, amount: parseFloat(e.target.value) || 0 })} /></div>
